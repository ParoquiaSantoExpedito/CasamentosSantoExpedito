...
<script>
  ...
  const feriados = {
    ...
    "2026-08-15": { nome: "Nossa Senhora da Glória (Padroeira de Maringá)", tipo: "feriado-municipal" },
    "2026-02-16": { nome: "Carnaval (facultativo)", tipo: "facultativo" },
    "2026-02-17": { nome: "Carnaval (facultativo)", tipo: "facultativo" },
    "2026-02-18": { nome: "Quarta-feira de Cinzas até 14h (facultativo)", tipo: "facultativo" },
    "2026-06-04": { nome: "Corpus Christi (facultativo)", tipo: "facultativo" },
    "2026-10-15": { nome: "Dia do Professor (facultativo)", tipo: "facultativo" },
    "2026-10-28": { nome: "Dia do Servidor Público (facultativo)", tipo: "facultativo" },
    "2026-12-24": { nome: "Véspera de Natal após 14h (facultativo)", tipo: "facultativo" },
    "2026-12-31": { nome: "Véspera de Ano Novo após 14h (facultativo)", tipo: "facultativo" }
  };
  ...
  async function salvarEstado(dateStr, novoEstado) {
    try {
      const res = await fetch("/api/estado", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({ dateStr, novoEstado })
      });
        ...
        if (dataObj.getDay() === 6) {
          ...
          div.addEventListener("click", async () => {
            ...
            if (senha === senhaCorreta) {
              ...
              await salvarEstado(dateStr, novoEstado);
            } else if (senha !== null) {
